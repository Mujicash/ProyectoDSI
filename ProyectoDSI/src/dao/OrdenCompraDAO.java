...
public class OrdenCompraDAO {
    ...
    public static void insertar(OrdenCompraDTO nuevo) {
        ...
        try ( PreparedStatement pst = conn.prepareStatement(sql)) {
            pst.setInt(1, nuevo.getProveedor().getIdProveedor());
    ...
    public static void modificar(OrdenCompraDTO modificado) {
        ...
        Connection conn = Conexion.getInstance().getConn();
        try ( PreparedStatement pst = conn.prepareStatement(sql)) {
            pst.setInt(1, modificado.getProveedor().getIdProveedor());
     ...
     */
    public static void eliminar(OrdenCompraDTO eliminar) {//Quizas solo pedir la id
        String sql = "DELETE FROM tbl_orden_compra WHERE id_compra=?";
        Connection conn = Conexion.getInstance().getConn();
        try ( PreparedStatement pst = conn.prepareStatement(sql)) {
            pst.setInt(1, eliminar.getIdOrdenCompra());
            pst.executeUpdate();
        } catch (SQLException ex) {
     ...
     */
    /*
    public static OrdenCompraDTO buscar(int id) {
